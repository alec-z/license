...
esURL = os.getenv("ES_URL") or ''
esPassword = os.getenv("ES_PASSWORD") or ''
esUser = 'elastic'
finalURL = None
if esURL.startswith('https://'):
    finalURL = 'https://' + esUser + ':' + esPassword + '@' +esURL[8:]
if esURL.startswith('http://'):
    finalURL = 'http://' + esUser + ':' + esPassword + '@' +esURL[7:]
if esURL:
    elasticSearch = Elasticsearch([finalURL], verify_certs= False)
